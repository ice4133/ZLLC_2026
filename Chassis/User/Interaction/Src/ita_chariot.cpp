...
{   
    ...
    switch(CAN_Manage_Object->Rx_Buffer.Header.Identifier)
    {
        case (0x77):
        {
            ...
            gimbal_velocity_x = Math_Int_To_Float(tmp_velocity_x,0,10000,-4.f,4.f);
            gimbal_velocity_y = Math_Int_To_Float(tmp_velocity_y,0,10000,-4.f,4.f);
            chassis_omega = Math_Int_To_Float(tmp_omega, 0, 10000, -4.f, 4.f);      // Chassis_Radius;//映射范围除以五十 云台发的是车体角速度 转为舵轮电机的线速度
...
void Class_Chariot::Control_Chassis_Test()
{
    ...
    // 云台坐标系的目标速度转为底盘坐标系的目标速度 正常情况下不会更正，只有有偏移未矫正或者小陀螺或反小陀螺会更正
    //无论云台如何动，dt7传的数据可以直接对应底盘车体运动行进，而不是完全以头为正方向
    chassis_velocity_x = 1.0f * ((float)(chassis_velocity_x * cos(derta_angle) - chassis_velocity_x * sin(derta_angle)));
    chassis_velocity_y = 1.0f * ((float)(chassis_velocity_y * sin(derta_angle) + chassis_velocity_y * cos(derta_angle)));
    if(chassis_omega < 0.5f && chassis_omega > -0.5f)chassis_omega = 0;//限幅
    //小陀螺行进，我感觉一般不会进入这个判断
